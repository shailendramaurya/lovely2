...
if (bitrate_i == 4) {quality = 320} else {quality = 160;}
    ...
    results_objects[song_id] = {
      ...
      results.push(`
        <div class="text-left song-container" style="margin-bottom: 20px;border-radius: 10px; background-color:#FFDAD2; padding: 10px; border:1px #FFDAD2;box-shadow:-5px -5px 30px black;">
        <div class="row" style="margin: auto;">
            <div class="col-auto" style="padding: 0px;padding-right: 0px;border-style: none;">
            <img id="${song_id}-i" class="img-fluid d-inline" style="width: 115px;border-radius: 10px;height: 115px;padding-right:10px;" src="${song_image}" loading="lazy" /></div>
            <div class="col" style="border-style: none;padding: 2px;">
                <p class="float-right fit-content" style="margin: 0px;color:black;padding-right:10px;">${year}</p>
                <p id="${song_id}-n" class="fit-content" style="margin: 0px;color:black;max-width: 100%;">${song_name}</p>
                <p id="${song_id}-a" class="fit-content" style="margin: 0px;color:black;max-width: 100%;">${album_name}<br /></p>
                <p id="${song_id}-ar" class="fit-content" style="margin: 0px;color:black;max-width: 100%;">${song_artist}<br /></p>
                <button class="btn btn-color" type="button" style="margin: 0px 2px;" onclick='PlayAudio("${download_url}","${song_id}")'>▶</button>
                <button class="btn btn-color" type="button" style="margin: 0px 2px;" onclick='AddDownload("${song_id}")'><img src="https://img.icons8.com/ios/50/000000/download.png" width="50%"></button>
                <p class="float-right fit-content" style="margin: 0px;color:black;padding-right:10px;padding-top:15px;">${play_time}<br /></p>
            </div>
        </div>
    </div>
`
